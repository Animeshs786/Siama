  ...
  try {
    ...
    for (let i = 0; i < scat_arr.length; i++) {
      ...
      if (!isValidObjectId(scat_id))
        throw new ApiError(`Invalid sub category ID: ${scat_id}`, 400);
      ...
      if (scateg.category.toString() === "6654515f16a3fd61d1e6e101") {
        maleSubCat.push(scat_id);
      }

      if (scateg.category.toString() === "665451b416a3fd61d1e6e104") {
        femaleSubCat.push(scat_id);
      }
    }
    ...
    vendor.sub_categories_male=maleSubCat;
    vendor.sub_categories_female=femaleSubCat;
